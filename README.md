...
use AmoCRM\Unsorted;
```
Далее, в любом месте перед использованием обёртки, нужно подключить файл автозагрузки классов 'autoload.php'. Путь к файлу конечно же может отличаться от примера ниже. Или вы можете всключать классы в свой автозагрузчик.
```php
<?php require_once 'AmoCRM_Wrap/autoload.php'; ?>
```
## Использование
Для начала нужно произвести авторизацию. Для этого создаётся экземпляр класса Амо в конструктор которого необходимо передать:

1. Субдомен црм
2. Логин юзера(почта)
3. API ключ пользователя. Его вы можете найти в интерфейсе црм https://test.amocrm.ru/settings/profile/ - 'test' заменить на свой поддомен
...
* Получение списка любых сущностей с различными фильтрами
...
## Класс Amo
* **Amo::VERSION** —Текущая версия обёртки
* **Amo::clearPhone($phone)** — Статичный метод отчистки телефона от лишних символов.
...
* **contactsList($query = null, $limit = 0, $offset = 0, $responsibleUsersIdOrName = array(), \DateTime $modifiedSince = null, $isRaw = false)** — Производит поиск по контактам и возвращает массив объектов класса Contact. Параметры: $query - произвольная строка, $limit - лимит количества объектов в результате, $offset - отступ от начала списка, $responsibleUsersIdOrName - один элемент или массив ответственных (принимает как id так и имена или часть имени), $modifiedSince - дата и время после которых контакт был изменён, $isRaw - если true то возвращает массив сырых данных, а не массив объектов
* **leadsList($query = null, $limit = 0, $offset = 0, $responsibleUsersIdOrName = array(), \DateTime $modifiedSince = null, $isRaw = false)** — Производит поиск по сделкам и возвращает массив объектов класса Lead. Параметры: $query - произвольная строка, $limit - лимит количества объектов в результате (техническое ограничение не более 0), $offset - отступ от начала списка, $responsibleUsersIdOrName - один элемент или массив ответственных (принимает как id так и имена или часть имени), $modifiedSince - дата и время после которых сделка была изменена, $isRaw - если true то возвращает массив сырых данных, а не массив объектов
* **companyList($query = null, $limit = 0, $offset = 0, $responsibleUsersIdOrName = array(), \DateTime $modifiedSince = null, $isRaw = false)** — Производит поиск по компаниям и возвращает массив объектов класса Company. Параметры: $query - произвольная строка, $limit - лимит количества объектов в результате, $offset - отступ от начала списка, $responsibleUsersIdOrName - один элемент или массив ответственных (принимает как id так и имена или часть имени), $modifiedSince - дата и время после которых компания была изменена, $isRaw - если true то возвращает массив сырых данных, а не массив объектов
* **tasksList($query = null, $limit = 0, $offset = 0, $responsibleUsersIdOrName = array(), \DateTime $modifiedSince = null, $isRaw = false)** — Производит поиск по задачам и возвращает массив объектов класса Task. Параметры: $query - произвольная строка, $limit - лимит количества объектов в результате, $offset - отступ от начала списка, $responsibleUsersIdOrName - один элемент или массив ответственных (принимает как id так и имена или часть имени), $modifiedSince - дата и время после которых задача была изменена, $isRaw - если true то возвращает массив сырых данных, а не массив объектов
* **notesContactList($query = null, $limit = 0, $offset = 0, $responsibleUsersIdOrName = array(), \DateTime $modifiedSince = null, $isRaw = false)** — Производит поиск по заметкам у контатов и возвращает массив объектов класса Note. Параметры: $query - произвольная строка, $limit - лимит количества объектов в результате, $offset - отступ от начала списка, $responsibleUsersIdOrName - один элемент или массив ответственных (принимает как id так и имена или часть имени), $modifiedSince - дата и время после которых задача была изменена, $isRaw - если true то возвращает массив сырых данных, а не массив объектов
* **notesLeadList($query = null, $limit = 0, $offset = 0, $responsibleUsersIdOrName = array(), \DateTime $modifiedSince = null, $isRaw = false)** — Производит поиск по заметкам у сделок и возвращает массив объектов класса Note. Параметры: $query - произвольная строка, $limit - лимит количества объектов в результате, $offset - отступ от начала списка, $responsibleUsersIdOrName - один элемент или массив ответственных (принимает как id так и имена или часть имени), $modifiedSince - дата и время после которых задача была изменена, $isRaw - если true то возвращает массив сырых данных, а не массив объектов
* **notesCompanyList($query = null, $limit = 0, $offset = 0, $responsibleUsersIdOrName = array(), \DateTime $modifiedSince = null, $isRaw = false)** — Производит поиск по заметкам у компаний и возвращает массив объектов класса Note. Параметры: $query - произвольная строка, $limit - лимит количества объектов в результате, $offset - отступ от начала списка, $responsibleUsersIdOrName - один элемент или массив ответственных (принимает как id так и имена или часть имени), $modifiedSince - дата и время после которых задача была изменена, $isRaw - если true то возвращает массив сырых данных, а не массив объектов
* **notesTaskList($query = null, $limit = 0, $offset = 0, $responsibleUsersIdOrName = array(), \DateTime $modifiedSince = null, $isRaw = false)** — Производит поиск по заметкам у задач(рехультат задачи) и возвращает массив объектов класса Note. Параметры: $query - произвольная строка, $limit - лимит количества объектов в результате, $offset - отступ от начала списка, $responsibleUsersIdOrName - один элемент или массив ответственных (принимает как id так и имена или часть имени), $modifiedSince - дата и время после которых задача была изменена, $isRaw - если true то возвращает массив сырых данных, а не массив объектов
...
* **getAmoId()** — Возвращает id сущности
...
* **getDateUpdate()** — Возвращает объект DateTime с датой и временем последнего изменения сущности
...
* **addLeadId($leadId)** — Прикрепляет к сущности сделку по id
* **delLeadId($leadId)** — Открепляет сделку от сущности по id
* **getContactsId()** — Возвращает массив из id прикреплённых контактов
* **addContactId($contactId)** — Прикрепляет к сущности контакт по id
* **delContactId($contactId)** — Открепляет контакт от сущности по id
* **getCompanyId()** — Возвращает id компании к которой привязана сущность
* **setCompanyId($companyId)** — Устанавливает id компании к которой будет привязана сущность
...
* **getCustomFieldValue($nameOrId)** — Принимает имя кастомного поля или его id. Возвращает его значение в виде строки. Если значений несколько, то они перечесляются через точку с запятоу (;)
* **getCustomFieldsValue()** — Возвращает все значения кастомных полей в виде ассоциативного массива (имя поля => его значение). Если у поля несколько значений, то они перечесляются через точку с запятоу (;)
...
* **addFile($pathToFile)** — прикрепляет файл к сущности (Сделка или Контакт), в $pathToFile необходимо передать путь до файла
* **backup($directory)** — Создаёт полный бэкап всей црм в директории $directory
...
* **setMainContactId($contactId)** — Задаёт id основного контакта
...
if ($amo->isAuthorization()) {
    ...
    $lead->setSale(2500); //Создаём сделку, которая будет создана в црм после принятия заявки в неразобранном
    $unsorted = new Unsorted('Форма на лендосе', $lead, array($contact, $contact2), 'Вторые продажи');
...
if ($amo->isAuthorization()) {
    ...
    if ($isSaveLead) {
        $contact->addLeadId($lead->getAmoId());
    }
    $contact->save();
}
?>
